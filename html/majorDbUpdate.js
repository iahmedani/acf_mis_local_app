...
fs.stat(`${process.env.APPDATA}/nims_aapacf_mis_local_b4Update.db`, async (err, stat) => {
    if (err) {
        try {
            fs.copyFileSync(`${process.env.APPDATA}/nims_aapacf_mis_local.sqlite3`, `${process.env.APPDATA}/nims_aapacf_mis_local_b4Update.db`, 1);
            Swal.fire({
                type: 'success',
                title: 'Oops...',
