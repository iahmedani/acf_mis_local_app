exports.up = function (knex, Promise) {
      ...
      `
        ...
        [total_exits] INTEGER DEFAULT 0, 
        [ent_type] CHAR(10), 
        [total_scr_lactating2] INTEGER, 
        [new_scr_lactating2] INTEGER, 
        [reScreened_scr_lactating2] INTEGER, 
        [muac_gt_21_lactating2] INTEGER, 
        [muac_le_21_lactating2] INTEGER, 
        [ifa_first_time_lactating2] INTEGER, 
        [total_adolescent] INTEGER);`
              )
              .raw(
      ...
      FROM   [Screening]
             INNER JOIN [v_geo] ON [Screening].[site_id] = [v_geo].[site_id];`
              ...
              .raw(`CREATE VIEW [scr_report_final]
    AS
    SELECT 
           [province_id], 
           [district_id], 
    ...
    `)
      ...
      WHERE  [tblsessions].[is_deleted] = 0;`
              ...
              .raw(
                     `CREATE VIEW [vSessionForReportNew]
      AS
      SELECT 
    ...
    GROUP  BY
      ...
      FROM   [main].[v_geo]
             INNER JOIN [main].[tblOtpAdd] ON [main].[v_geo].[site_id] = [main].[tblOtpAdd].[site_id];`
              ...
              .raw(
                     `CREATE VIEW [v_OtpAdd_yearmonth]
      AS
      SELECT 
             [site_id], 
             STRFTIME ('%Y-%m', [reg_date]) AS [Year_month], 
...
WHERE  [tblOtpExit].[is_deleted] = 0
      ...
      WHERE  [main].[tblOtpExit].[is_deleted] = 0
              ...
              .raw(`CREATE VIEW [v_otpExitFull_report]
    AS
    SELECT 
           [province_id], 
           [province], 
    ...
    GROUP  BY [prog_type];`)
              ...
              .raw(`CREATE VIEW [v_otpExitReportNew]
    AS
    SELECT 
           [main].[v_geo].[province_id], 
           [main].[v_geo].[province], 
    ...
    FROM   [main].[v_geo]
           ...
           INNER JOIN [main].[tblOtpExit] ON [main].[tblOtpExit].[otp_id] = [main].[tblOtpAdd].[otp_id];`)
              .raw(`CREATE VIEW [v_otpExit_knex]
    AS
    SELECT 
           [v_geo].*, 
           [v_OtpExit].*
    FROM   [main].[v_geo]
           INNER JOIN [main].[v_OtpExit] ON [main].[v_OtpExit].[site_id] = [main].[v_geo].[site_id];`)
              .raw(`CREATE VIEW [v_OtpExit_yearmonth]
    AS
    SELECT 
           [main].[tblOtpAdd].[site_id], 
           STRFTIME ('%Y-%m', [main].[tblOtpExit].[exit_date]) AS [Year_month], 
    ...
    WHERE  [main].[tblOtpExit].[exit_id] IS NULL
             AND [main].[tblOtpAdd].[prog_type] = 'otp'
             AND [main].[tblOtpAdd].[is_deleted] = 0;`)
              .raw(`CREATE VIEW [v_otpNotExitInterval]
    AS
    SELECT 
           [v_otpNotExit].*, 
           [main].[v_geo].[province_id], 
    ...
    WHERE  [main].[tblOtpAdd].[is_deleted] = 0
             AND [main].[tblOtpAdd].[prog_type] = 'otp';`)
              .raw(`CREATE VIEW [v_otp_remaining]
    AS
    SELECT 
    ...
    GROUP  BY
              ...
              [main].[v_OtpAdd_yearmonth].[age_grp], 
              [main].[v_OtpAdd_yearmonth].[gender];`)
              .raw(`CREATE VIEW [v_otp_remaining_geo]
    AS
    SELECT 
           [main].[v_geo].[province_id], 
           [main].[v_geo].[district_id], 
    ...
    FROM   [main].[v_outreach_ind]
           ...
           LEFT JOIN [main].[tblNSC] ON [main].[tblNSC].[nsc_id] = [main].[v_outreach_ind].[ref_nsc];`)
              .raw(`CREATE VIEW [v_scrChildFull]
           AS
           SELECT 
                  [main].[v_geo].[province_id], 
                  [main].[v_geo].[province], 
           ...
           WHERE  [tblScrPlw].[is_deleted] = 0;`)
              .raw(`CREATE VIEW [v_session_full]
           AS
           SELECT 
                  [v_geo].*, 
                  [tblSessions].*
      ...
      GROUP  BY
                ...
                [month], 
                [Year];`)
              .raw(`CREATE VIEW [v_StockMovement]
      AS
      SELECT 
      ...
      `)
              .raw(`CREATE VIEW [v_stockMovements]
      AS
      SELECT 
             [year], 
             [month], 
      ...
      SELECT 
             ...
             'Stock' [source]
      FROM   [v_stockIn];`)
              .raw(`CREATE VIEW [v_StockMovementv2]
      AS
      SELECT 
             IFNULL ([item_name], [ration]) AS [item], 
             IFNULL ([rec_qty], 0) AS [recQty], 
      ...
      FROM   ([tblSupervisors]
             ...
             INNER JOIN [tblGeoProvince] ON [tblGeoDistrict].[province_id] = [tblGeoProvince].[id];
      `)
              .raw(`CREATE VIEW [v_tblScrChildrenFull]
      AS
      SELECT 
             [main].[v_geo].[province], 
             [main].[v_geo].[district_name], 
      ...
      FROM   [main].[tblScrChildren]
             INNER JOIN [main].[v_geo] ON [main].[tblScrChildren].[site_id] = [main].[v_geo].[site_id];`)
              .raw(`CREATE VIEW [v_tblScrPlwFull]
      AS
      SELECT 
             [main].[v_geo].[province], 
             [main].[v_geo].[district_name], 
...
exports.down = function (knex, Promise) {
       return knex.schema
              ...
              .raw('drop view v_geo')
              .raw('drop table tblVillages')
              .raw('drop table tblVillage')
              .raw('drop table tblSupervisors')
              .raw('drop table tblStokDistv2')
              .raw('drop table tblStockRequest')
              .raw('drop table tblStock')
              .raw('drop table tblSiteStock')
              .raw('drop table tblSessions')
              .raw('drop table tblScrStockDist')
              .raw('drop table tblScrPlw')
              .raw('drop table tblScrChildren')
              .raw('drop table tblOutreachInd')
              .raw('drop table tblOtpFollowup')
              .raw('drop table tblOtpExit')
              .raw('drop table tblOtpAdd')
              .raw('drop table tblNSC')
              .raw('drop table tblLhw')
              .raw('drop table tblInterimOtp')
              .raw('drop table tblGeoUC')
              .raw('drop table tblGeoTehsil')
              .raw('drop table tblGeoProvince')
              .raw('drop table tblGeoNutSite')
              .raw('drop table tblGeoDistrict')
              .raw('drop table tblConfig')
              .raw('drop table tblCommodity')
              .raw('drop table Screening')
};